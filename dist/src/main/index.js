...
async function wechatStart() {
    ...
    activeWeChatLogins.set(sessionKey, { qrcode: data.qrcode, qrcodeUrl: qrDataUrl, startedAt: Date.now(), refreshCount: 0 });
...
async function wechatStatus(sessionKey) {
    ...
    try {
        const res = await fetch(url, { headers: { 'iLink-App-ClientVersion': '1' }, signal: ctrl.signal });
        const result = await res.json();
        if (result.status !== 'wait')
            console.log(`[WeChat] status=${result.status}${result.redirect_host ? ` redirect=${result.redirect_host}` : ''}`);
        if (result.status === 'expired') {
            login.refreshCount++;
            if (login.refreshCount > 3) {
                activeWeChatLogins.delete(sessionKey);
    ...
    return new Promise((resolve, reject) => {
        const server = http.createServer(async (req, res) => {
            ...
            if (url.pathname === '/api/wechat/start' && req.method === 'POST') {
                try { const result = await wechatStart(); res.writeHead(200, { 'Content-Type': 'application/json' }); res.end(JSON.stringify(result)); }
            ...
            if (url.pathname === '/api/wechat/status' && req.method === 'GET') {
                ...
                try { const result = await wechatStatus(session); res.writeHead(200, { 'Content-Type': 'application/json' }); res.end(JSON.stringify(result)); }
                catch (err) { res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: err.message })); }
                return;
            }
