...
function nameToDirName(name) {
    return name.toLowerCase().replace(/\s+/g, '-');
}
function copyWorkspaceFiles(sourcePath, targetPath) {
...
export class WorkspaceManager {
    register() {
        ...
        ipcMain.handle(IPC_CHANNELS.WORKSPACE_CREATE, async (_e, name) => this.create(name));
        ipcMain.handle(IPC_CHANNELS.WORKSPACE_DELETE, async (_e, id) => this.delete(id));
        ipcMain.handle(IPC_CHANNELS.WORKSPACE_RENAME, async (_e, id, name) => this.rename(id, name));
        ipcMain.handle(IPC_CHANNELS.WORKSPACE_EXPORT, async (_e, id) => this.export(id));
        ipcMain.handle(IPC_CHANNELS.WORKSPACE_IMPORT, async (_e, filePath) => this.import(filePath));
        ipcMain.handle(IPC_CHANNELS.WORKSPACE_CLONE, async (_e, id, name) => this.clone(id, name));
    }
    create(name) {
    ...
    delete(id) {
        ...
        try {
            ...
            pushLog('workspace', `磁盘清理失败：${workspace.name} — ${err.message}`, LogLevel.WARN);
        }
        saveWorkspaces(store.workspaces);
    ...
    rename(id, name) {
        ...
        try {
            ...
            pushLog('workspace', `磁盘重命名失败：${workspace.name} — ${err.message}`, LogLevel.WARN);
        }
    ...
    import(filePath) {
        try {
            ...
            if (Array.isArray(data.agents) && data.agents.length > 0) {
                const importedAgents = data.agents.map((a) => ({
                    ...a,
                    id: createId('agent'),
    ...
    clone(id, name) {
        ...
        try {
            ...
            pushLog('workspace', `磁盘克隆失败：${source.name} — ${err.message}`, LogLevel.WARN);
        }
        const newWorkspace = {
            id: createId('workspace'),
            name: trimmedName,
