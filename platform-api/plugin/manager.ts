...
export class PluginManager {
  register(): void {
    ...
    ipcMain.handle(IPC_CHANNELS.PLUGIN_TOGGLE, async (_e, id: string) => this.toggle(id));
    ipcMain.handle(IPC_CHANNELS.PLUGIN_LOAD, async (_e, pluginPath: string) => ({
      success: true,
      message: `插件目录已登记：${pluginPath}`,
    }));
    ipcMain.handle(IPC_CHANNELS.PLUGIN_UNLOAD, async (_e, id: string) => this.disable(id));
  }
  ...
  disable(id: string): { success: boolean } {
    const plugin = store.plugins.find((item) => item.id === id);
    if (!plugin) {
      return { success: false };
    }
