...
import { ChannelConfig, ChannelType } from '../../shared/types.js';
...
export class ChannelManager {
  private channels: ChannelConfig[] = loadChannels();

  register(): void {
    ipcMain.handle(IPC_CHANNELS.CHANNEL_LIST, () => this.channels);

    ipcMain.handle(IPC_CHANNELS.CHANNEL_SAVE, (_event, id: string, config: Record<string, string>) => {
    ...
    ipcMain.handle(IPC_CHANNELS.CHANNEL_TOGGLE, (_event, id: string) => {
    ...
    ipcMain.handle(IPC_CHANNELS.CHANNEL_TEST, (_event, id: string) => {
      const ch = this.channels.find((c) => c.id === id);
      if (!ch) return { success: false, message: 'Channel not found' };
      if (ch.type === 'wechat') return { success: false, message: '微信渠道暂未支持' };

      // Basic validation: check required fields
      const missing: string[] = [];
      if (ch.type === 'telegram' && !ch.config.token) missing.push('Token');
      if (ch.type === 'lark' && (!ch.config.appId || !ch.config.appSecret)) missing.push('AppID / AppSecret');
      if (ch.type === 'matrix' && (!ch.config.homeserver || !ch.config.accessToken)) missing.push('Homeserver / Access Token');
